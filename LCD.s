...
; Last modification date: 4/15/2019
...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	LDR R2, =SSI0_SR_R
wait1
	LDR R1,[R2]						;	Read bit 4 of SSI0_SR_R
	AND R1,#0x10
	LSR R1,#4
	CMP R1,#0
	BNE wait1						;	If bit 4 high, loop back
	LDR R2,=GPIO_PORTA_DATA_R
	LDR R1,[R2]
	AND R1,#0xBF
	STR R1,[R2]						;	Clear D/C
	LDR R2,=SSI0_DR_R
	STR R0,[R2]						;	Write command to SSI0_DR_R
	LDR R2, =SSI0_SR_R
wait2
	LDR R1,[R2]						;	Read bit 4 of SSI0_SR_R
	AND R1,#0x10
	LSR R1,#4
	CMP R1,#0
	BNE wait2						;	If bit 4 high, loop back
    BX  LR                          ;   return
...
;4) Write the 8-bit data to SSI0_DR_R
	LDR R2, =SSI0_SR_R
loop1
	LDR R1,[R2]						;	Read bit 1 of SSI0_SR_R
	AND R1,#0x2
	LSR R1,#1
	CMP R1,#0
	BEQ loop1						;	If bit 1 high, loop back
	LDR R2,=GPIO_PORTA_DATA_R
	LDR R1,[R2]
	ORR R1,#0x40
	STR R1,[R2]						;	Set D/C to high
	LDR R2,=SSI0_DR_R
	STR R0,[R2]						;	Write data to SSI0_DR_R
    BX  LR                          ;   return
